...
final class ChatViewModel: ObservableObject {
    ...
    private func runLoop(app: AppState) async {
        ...
        Market tool pack: indicator_values(symbol,timeframe) full indicator snapshot; divergence_scan(symbol,timeframe[,indicator]); spike_scan(symbol,timeframe); fib_levels(symbol,timeframe[,lookback] or high,low[,direction]); pivot_levels(symbol,timeframe[,method]); streak_stats(symbol,timeframe); candle_anatomy(symbol,timeframe[,count]); volatility_rank([symbols,timeframe]); session_clock(); risk_reward(entry,stop,target[,account_size,risk_percent]); kelly_size(win_rate,payoff[,account_size]); watchlist_show(); watchlist_add(symbols); watchlist_remove(symbols). \
