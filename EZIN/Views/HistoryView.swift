...
struct HistoryView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 14) {
                ...
                if mode == 0 { tradesSection } else { signalsSection }
            }
    ...
    // MARK: Generated signals (works with no API token)
    private var signalsSection: some View {
        VStack(spacing: 14) {
            HStack {
                Text("Generated signals").font(.caption).foregroundStyle(.white.opacity(0.5))
                Spacer()
                if !signalLog.signals.isEmpty {
                    Button("Clear") { signalLog.clear() }.font(.caption).foregroundStyle(Glass.sell)
                }
            }
            if signalLog.signals.isEmpty {
                EmptyState(icon: "waveform.path.ecg",
                           title: "No signals recorded yet",
                           subtitle: "As the council generates signals they are logged here in real time — no API token required.")
            } else {
                ForEach(signalLog.signals) { s in SignalCard(signal: s) }
            }
        }
    }
